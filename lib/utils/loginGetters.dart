  ...
  static Future<bool> getEverythingAndLogin(
    ...
    try {
      ...
      if (log.statusCode == 200) {
        ...
        if (kIsWeb) {
          document.cookie =
              "token=${decodedResponse["payload"]["access_token"]}";
          Utils.logger.i("HEADERS:  ${log.headers}");
        } else {
          clientProvider
              .setAccessToken(decodedResponse['payload']['access_token']);
        }
        ...
        final apiFriendRequests =
            await Api.getFriendRequests(clientId: clientId);
        ...
        for (int i = 0; i < groups.length; i++) {
          ...
          groupIcons
              .add(DouchatNotificationIcon(id: groups[i].id, bytes: bytes));
        }
      ...
      Utils.logger.i("Something went wrong", e, s);
  ...
  static Future<bool> getEverythingAndRegister(
    ...
    if (res.statusCode == 200) {
      ...
      final clientProvider =
          Provider.of<ClientProvider>(context, listen: false);
      if (kIsWeb) {
        document.cookie = "token=${decoded["token"]}";
      } else {
